...
* 环境
  ...
  RUN apt-get -y update && apt-get install -y --no-install-recommends \
      autoconf automake libtool gawk cpio lzop kmod python python3 mtd-utils libncurses5-dev pkg-config libgcrypt-dev \
      vim git openssh-server zip unzip bc wget build-essential device-tree-compiler u-boot-tools squashfs-tools make gcc g++ \
...
modprobe nvp6124_kdrv drv_mode=0 dev_num=2 ibus=0,0 iaddr=0x60,0x62 vout_mode=4,4 vout_xcap=0x03040000,0x01020000 vout_vi=0x03040000,0x01020000 vout_cdly=0x08080000 clk_dly=0x00000000,0x00000000 clk_inv=0x01010000,0x01010000 ch_map=2 clk_used=0x1 rstb_used=106 clk_driving=2
```
...
| vout_xcap   | bit0 ~ 7: 0 ~ 12<br/>bit8 ~ 15: 0 ~ 12<br/>bit16 ~ 23: 0 ~ 12<br/>bit24 ~ 31: 0 ~ 12<br/>(0:None<br/>1 ~ 8:XCAP0~7(BT656)<br/>9:XCAP0+XCAP1(BT1120)<br/>10:XCAP2+XCAP3(BT1120)<br/>11:XCAP4+XCAP5(BT1120)<br/>12: XCAP6+XCAP7(BT1120)) | 设定每个 decoder 中，每个 output port 要如何连接到<br/>video capture port |
| vout_vi     | bit0 ~ 7: 0 ~ 8<br/>bit8 ~ 15: 0 ~ 8<br/>bit16 ~ 23: 0 ~ 8<br/>bit24 ~ 31: 0 ~ 8<br/>(0:None<br/>1 ~ 8:VI#0 ~ 7, only support<br/>XCAP#0->VI#0, XCAP#1->VI#1,<br/>XCAP#2->VI#2, XCAP#3->VI#3……) | 设定每个 decoder 中，每个 output port 要透过哪个<br/>capture vi 来接收影像 |
| vout_enb    | bit0 ~ 7: 0 ~ 1<br/>bit8 ~ 15: 0 ~ 1<br/>bit16 ~ 23: 0 ~ 1<br/>bit24 ~ 31: 0 ~ 1 | 设定每个 decoder 中，每个 output port 的 enable              |
| vout_cdly   | bit0 ~ 7: 0 ~ 15<br/>bit8 ~ 15: 0 ~ 15<br/>bit16 ~ 23: 0 ~ 15<br/>bit24 ~ 31: 0 ~ 15 | 设定每个 decoder 中，每个 output port 的 clock output delay<br/>(由 decoder 本身做 delay) |
| clk_dly     | bit0 ~ 7: 0 ~ 15<br/>bit8 ~ 15: 0 ~ 15<br/>bit16 ~ 23: 0 ~ 15<br/>bit24 ~ 31: 0 ~ 15 | 设定每个 decoder 中，每个 output port 的 receive clock delay<br/>(由 32x 做 delay) |
...
| clk_pin     |                                                              | 设定 AD vout 是接到哪一组 capture port, 而此 capture<br/>port 的 clock 是要选用哪一根 pin, 分 bt656 和 bt1120 的<br/>case |
| data_swap   | bit0 ~ 7: 0 ~ 1<br/>bit8 ~ 15: 0 ~ 1<br/>bit16 ~ 23: 0 ~ 1<br/>bit24 ~ 31: 0 ~ 1<br/>(0:disable 1:enable) | 设定每个 decoder 中，每个 output port 的 data bit order<br/>是否要 swap |
...
| clk_src     | 0~3<br/>0: Display1(MPLL10), 1:MAC(MPLL7),<br/>2: SSP(MPLL12), 3: OSC | 设定 32x 提供给 decoder 的 external clock 之 clock<br/>source |
