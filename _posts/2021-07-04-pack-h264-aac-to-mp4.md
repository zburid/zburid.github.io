...
<text x="300" y="40" font-size="24" font-family="Times New Roman" font-weight="bold" fill="dark">NALU payload(EBSP)</text>
</svg>
...
`nal_unit_type`：指定了`NALU`数据体中包含的`RBSP`数据结构类型。`1`到`5`范围的类型值表示`VCL`类型的`NALU`数据，具体的说明如下表中所示：

| type  | NALU内容<br/>RBSP语法结构                                    | C     | NALU类型<br/>[Annex A] | NALU类型<br/>[Annex G & H] | NALU类型<br/>[Annex I] |
| ----- | ------------------------------------------------------------ | ----- | ---------------------- | -------------------------- | ---------------------- |
